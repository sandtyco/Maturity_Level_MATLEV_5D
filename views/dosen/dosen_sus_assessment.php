...
if (!isset($_SESSION['user_id']) || $_SESSION['role_id'] != 2) { // role_id 2 untuk dosen
    header('Location: ../../login.php?error=Akses tidak diizinkan. Hanya dosen yang bisa mengakses halaman ini.');
    exit();
}

$user_id = $_SESSION['user_id'];
$username = $_SESSION['username']; // Biasanya ini adalah username login dosen
$full_name = $_SESSION['full_name'] ?? 'Dosen'; // Nama lengkap dosen
$role_name = $_SESSION['role_name']; // 'Dosen'
$is_asesor = $_SESSION['is_asesor'] ?? 0; // Tambahkan ini jika relevan untuk sidebar dosen

$message = '';
$error = '';
$selected_scores = []; // Untuk menyimpan skor yang sudah dipilih untuk pengisian ulang form

// --- CSRF Protection: Generate a new token on each page load ---
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // --- CSRF Protection: Validate the token on form submission ---
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        $error = "Terjadi kesalahan keamanan. Silakan coba lagi.";
        // Penting: Generate token baru jika validasi gagal untuk mencegah serangan berulang
        $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
    } else {
        // Reset token setelah digunakan untuk setiap kali submit form
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            if (isset($_POST[$question_name]) && in_array((int)$_POST[$question_name], [1, 2, 3, 4, 5])) {
                $scores[$question_name] = (int)$_POST[$question_name];
                $selected_scores[$question_name] = (int)$_POST[$question_name]; // Simpan untuk pengisian ulang
            } else {
                $all_questions_answered = false;
                $error = "Mohon jawab semua pertanyaan sebelum mengirimkan asesmen."; // Pindahkan pesan error kesini
                break; // Hentikan loop jika ada pertanyaan yang tidak terjawab
            }
        }

        if ($all_questions_answered) { // Lanjutkan hanya jika semua pertanyaan terjawab
            try {
                // Cek apakah user (dosen) sudah pernah melakukan asesmen pada tanggal yang sama
                ...
                if ($stmt_check->fetchColumn() > 0) {
                    ...
                    if ($stmt->execute($params)) {
                        ...
                        // Opsional: Redirect ke halaman hasil evaluasi dosen
                        // header('Location: dosen_sus_overall_results.php?status=success');
                        // exit();
                        // Kosongkan skor terpilih setelah sukses submit agar form bersih
                        $selected_scores = [];
                    } else {
                ...
                error_log("Error saving custom evaluation (dosen): " . $e->getMessage()); // Log detail error ke server
                $error = "Terjadi kesalahan sistem saat menyimpan evaluasi. Mohon coba beberapa saat lagi."; // Pesan umum untuk pengguna
            }
        }
    }
}
...
<head>
    ...
    <title>Formulir Evaluasi Aplikasi | Dosen - MATLEV 5D</title>
...
<body>
    ...
    <div id="sidebar">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="dosen_sus_assessment.php">
    ...
    <div id="content">
        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="card-body">
                ...
                <form method="POST" action="dosen_sus_assessment.php">
                    ...
                    <?php for ($i = 1; $i <= 4; $i++): ?>
                        <div class="mb-4 p-3 border rounded bg-light">
                            ...
                            <div class="d-flex flex-wrap align-items-center">
                                <?php for ($j = 1; $j <= 5; $j++): ?>
                                    <?php $current_q_name = 'q' . $i . '_score'; ?>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="<?php echo $current_q_name; ?>" id="<?php echo $current_q_name . $j; ?>" value="<?php echo $j; ?>" required
                                            <?php echo (isset($selected_scores[$current_q_name]) && $selected_scores[$current_q_name] == $j) ? 'checked' : ''; ?>>
                                        <label class="form-check-label" for="<?php echo $current_q_name . $j; ?>"><?php echo $j; ?> (<?php
                                            if ($j == 1) echo 'Sangat Tidak Setuju';
                                            else if ($j == 2) echo 'Tidak Setuju';
                                            else if ($j == 3) echo 'Netral';
                                            else if ($j == 4) echo 'Setuju';
                                            else if ($j == 5) echo 'Sangat Setuju';
                                        ?>)</label>
                                    </div>
                                <?php endfor; ?>
                            </div>
